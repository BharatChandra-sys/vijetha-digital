...
export default function WhatsAppButton() {
  ...
  // Show button after 1 second and tooltip after 2 seconds
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsVisible(true);
      // Show tooltip for 6 seconds after button appears
  ...
  // Initialize position - always use default bottom-right position
  useEffect(() => {
    const saved = localStorage.getItem('whatsapp-button-position');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        // Only use saved position if it's valid and not default
        if (parsed && typeof parsed.x === 'number' && typeof parsed.y === 'number' && (parsed.x !== 0 || parsed.y !== 0)) {
          setPosition(parsed);
        }
      } catch (e) {
  ...
  return (
    <>
      ...
      <div 
        ref={buttonRef}
        className="fixed bottom-4 right-4 lg:bottom-6 lg:right-6 flex flex-col items-end gap-3 sm:gap-4"
        style={{
          ...buttonStyle,
          display: 'flex',
          visibility: 'visible',
      ...
      >
        ...
        <button
          ...
          className={`group relative !rounded-full w-16 h-16 lg:w-20 lg:h-20 flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-200 ${isDragging ? 'cursor-grabbing scale-110' : 'cursor-grab hover:scale-105'} active:scale-95 touch-none`}
        ...
        >
          {/* WhatsApp Icon - Larger SVG with proper sizing */}
          <svg 
            viewBox="0 0 24 24" 
            fill="white"
          ...
          <span className="absolute -top-1 -right-1 text-white text-xs lg:text-sm font-bold rounded-full w-6 h-6 lg:w-7 lg:h-7 flex items-center justify-center shadow-sm" style={{ backgroundColor: '#E74C3C' }}>
            1
          </span>
